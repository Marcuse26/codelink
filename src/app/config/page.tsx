  ...
  return (
    <div className="p-6 space-y-8 max-w-2xl mx-auto">
      <div className="bg-white/5 p-6 rounded-2xl border border-white/10 space-y-3">
        ...
        <input className="w-full p-3 rounded-xl bg-black/30 text-white border border-white/10 outline-none focus:border-white/30 transition text-sm" placeholder="URL del Calendario" value={form.calendarUrl1} onChange={e => setForm({...form, calendarUrl1: e.target.value})} />
        <input className="w-full p-3 rounded-xl bg-black/30 text-white border border-white/10 outline-none focus:border-white/30 transition text-sm" placeholder="URL de Deporte (Strava...)" value={form.sportUrl1} onChange={e => setForm({...form, sportUrl1: e.target.value})} />
      </div>
      ...
      <div className="bg-white/5 p-6 rounded-2xl border border-white/10 space-y-3">
        ...
        <input className="w-full p-3 rounded-xl bg-black/30 text-white border border-white/10 outline-none focus:border-white/30 transition text-sm" placeholder="URL del Calendario" value={form.calendarUrl2} onChange={e => setForm({...form, calendarUrl2: e.target.value})} />
        <input className="w-full p-3 rounded-xl bg-black/30 text-white border border-white/10 outline-none focus:border-white/30 transition text-sm" placeholder="URL de Deporte (Strava...)" value={form.sportUrl2} onChange={e => setForm({...form, sportUrl2: e.target.value})} />
      </div>
      ...
      <button onClick={save} className="w-full py-4 bg-gradient-to-r from-blue-600 to-purple-600 rounded-xl text-white font-bold text-lg shadow-lg hover:scale-[1.02] transition-transform">GUARDAR CAMBIOS</button>
    </div>
  );
}